...
def get_att_model_1(nfa ,nsa , ip_max_len , op_max_len , eng_vocab_size, fren_vocab_size, dense1, dropout, activation1, activation2 , lr):
...
def get_att_model_2(nfa ,nsa , ip_max_len , op_max_len , eng_vocab_size, fren_vocab_size, dense1, dropout, activation1, activation2 , lr):
    ...
    last_densor = Dense(fren_vocab_size)
    activator = Activation(custom_softmax_2)
    ...
    for t in range(op_max_len + 1):    #+1 coz of eos in Y_mapped_output
        ...
        outputs.append(activate_output)

    model = Model(inputs = [inp , s0 , c0] , outputs = outputs)
    return model
